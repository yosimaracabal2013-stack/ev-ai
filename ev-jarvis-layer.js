...
(function(){
  'use strict';
  const frame = window.parent === window ? null : window.parent.document ? window.frameElement : null;
  const w = window;
  ...
  function openTab(name){
    const tab = Array.from(document.querySelectorAll('.tab')).find(b =>
      String(b.dataset.tab || '').toLowerCase() === name
    );
  ...
  function handle(text){
    ...
    if(/^wake up$|^wake up ev$/.test(q)){
      say('I am awake and ready. What do you need?');
      return true;
    }
    ...
    if(/^what time is it$|^what time is it right now$|^time$/.test(q)){
      say(`It is ${new Date().toLocaleTimeString([], {hour:'numeric', minute:'2-digit'})}.`);
      return true;
    }
    if(/^what'?s on the agenda$|^what is on the agenda$|^show my agenda$/.test(q)){
      say(agendaText());
      return true;
    }
    if(/^what do you remember$|^show memory$|^what do you have in memory$/.test(q)){
      say(memoryText());
      return true;
    }
    ...
    if(tm){
      let mins = Number(tm[1]);
      if((tm[2]||'').toLowerCase().startsWith('hour') || (tm[2]||'').toLowerCase().startsWith('hr')) mins *= 60;
      timer(mins);
      return true;
    }
    ...
    if(/^clear chat$/.test(q)){
      const b=document.getElementById('resetBtn'); if(b) b.click();
      return true;
    }
    return false;
  }

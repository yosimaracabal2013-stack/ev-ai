(function(){
  ...
  function explain(status,body){
    ...
    if(status===403)return 'Fish Audio denied this model request. I will try the available fallback before using the local voice.';
  ...
  function patch(){
    try{
      const w=frame.contentWindow;if(!w||w.__evFishRepairV4)return;w.__evFishRepairV4=true;
      ...
      w.speak=async function(text){
        ...
        catch(e){w.speechSynthesis?.cancel();add(w,'Fish Audio is not available right now. '+(e.message||'voice request failed')+' I kept your Fish key saved.');w.document.getElementById('voiceState').textContent='FISH ERROR';return false}
      };

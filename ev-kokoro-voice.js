/* E.V. NATURAL NEURAL VOICE v6
   ...
   The neural layer owns the voice selector so the iPhone fallback cannot overwrite it.
*/
(function(){
'use strict';
...
function menu(){const s=document.getElementById('voiceSelect');if(!s)return;s.dataset.evNeural='1';s.innerHTML='';for(const [id,label] of voices){const o=document.createElement('option');o.value=id;o.textContent=label;if(id===selected())o.selected=true;s.appendChild(o)}s.addEventListener('change',()=>{localStorage.setItem('ev-neural-voice',s.value);tts=null;loading=null});}
menu();
